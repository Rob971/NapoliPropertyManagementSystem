    ...
    <>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-border bg-background shadow-2xl animate-in slide-in-from-right duration-200">
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-5">
          ...
          <section className="space-y-3">
            <h3 className="text-sm font-medium text-muted-foreground">
              {t("bookingDetail.propertyCompliance")}
            </h3>
          ...
          {task && (
            <section className="space-y-3">
              <h3 className="text-sm font-medium text-muted-foreground">
                {t("bookingDetail.turnoverCleaning")}
              </h3>
              <div className="rounded-xl border p-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm">{t(`taskType.${task.type}`)}</p>
                  <span
                    className={cn(
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {t("bookingDetail.scheduled", { date: task.date })}
                </p>
                ...
                {task.status !== "Confirmed" && (
                  ...
                  >
                    ...
                    {isDispatching
                      ? t("bookingDetail.dispatching")
                      : t("bookingDetail.simulateDispatch")}
                  </Button>
                )}
              </div>
            </section>
